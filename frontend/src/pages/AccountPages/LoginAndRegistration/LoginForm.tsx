...
export default function LoginForm({ onLogin, toggleModal }: LoginProps) {
    const [loginInfo, setLoginInfo] = useState<LoginInfo>({ username: '', password: '', isIOS: isIOS() })
    ...
    const handleSubmit = async (evt: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.post(`${apiUrl}/api/accounts/login`, loginInfo, { withCredentials: true })
            if (isIOS()) {
            ...
            onLogin()
            toggleModal()
            setLoginInfo({ username: '', password: '', isIOS: false })
        } catch (err) {
